...
def should_get_message_with_higher_priority(priority_publisher_session: Publisher):
    test_data = []

    for i in range(0, 10):
        rand_int = randint(0, 255)
        body = {"test": f"test{rand_int}", "priority": rand_int}
        priority_publisher_session.publish(body, priority=rand_int)
    ...
    consumer = Consumer(
        exchange_name=priority_publisher_session.exchange_name,
        queue_name=priority_publisher_session.queue_name,
        routing_key=priority_publisher_session.routing_key,
        callback=callback,
...
def should_republish_message_to_original_queue_with_dlk_retry_enabled(
    ...
    consumer.close()
    cleanup_queue_and_exchange(consumer.retry_publisher)
...
def should_nack_message_when_callback_method_returns_false() -> None:
...
def should_consume_from_the_routed_queue_as_specified_in_headers() -> None:
    ...
    cleanup_queue_and_exchange(first_consumer)
    cleanup_queue_and_exchange(second_consumer)
